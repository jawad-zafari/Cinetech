 ...
 */
function updateUI(data, creditsData) {
    ...
    if (title)
        title.textContent = data.title || data.name || "Titre inconnu";
    if (poster && data.poster_path)
        poster.src = `${IMAGE_BASE_URL}${data.poster_path}`;
    if (overview)
        overview.textContent = data.overview || "Pas de description disponible.";
    // Année de sortie
    if (yearElement) {
    ...
    if (genresContainer && data.genres) {
        genresContainer.innerHTML = data.genres.map((g) => `<span>${g.name}</span>`).join('');
    }
}
/**
 * Récupère et affiche les acteurs principaux
 */
function loadCast(creditsData) {
